...
def get_arguments(epoch):
	...
	parser.add_argument("--valset_dir", type=str, default='/Data4/CASC/dataset_patch/Glo_Cell_aftercorrection/Test/data_list.csv')
	parser.add_argument("--snapshot_dir", type=str, default='snapshots_2D/PrPSeg_beforecorrection_CASC_e')
	parser.add_argument("--reload_path", type=str, default='snapshots_2D/PrPSeg_beforecorrection_CASC_e/MOTS_DynConv_PrPSeg_beforecorrection_CASC_e_e%d.pth' % (epoch))
		...
		with torch.no_grad():
			for iter, batch1 in enumerate(valloader):
				...
				output_folder = os.path.join(
					args.snapshot_dir.replace('snapshots_2D/', '/Data4/CASC/testing_'),
					str(epoch))
			...
			for now_task in range(task_num):  # Loop through tasks 0 to 22
				...
				if task_pool_image.num_imgs > 0:
					...
					if now_task in size_512:
						...
						preds = model(images, torch.ones(batch_size).cuda() * now_task,
											scales)
