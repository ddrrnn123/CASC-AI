...
class MOTSDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(64, 192, 192), mean=(128, 128, 128), scale=True,
        ...
        for ki in range(len(tasks)):
            # if os.path.basename(tasks[ki]) != '8_0_capsule' and os.path.basename(tasks[ki]) != '9_0_tufts':
            #     continue
            ...
            for si in range(len(stain_folders)):
                ...
                for ri in range(len(images)):
                    if ('mask' in images[ri]) or ('csv' in images[ri]) or ('PAS' in images[ri]):
                        continue
                    else:
                        image_root = images[ri]
                        _, ext = os.path.splitext(images[ri])

                        mask_root = glob.glob(os.path.join(stain_folders[si],os.path.basename(image_root).replace('img','mask')))[0]
                        # mask_root = 'aaa'
...
if __name__ == '__main__':
    ...
    root = '/Data4/CASC/dataset_patch/Glo_Cell_aftercorrection/'
